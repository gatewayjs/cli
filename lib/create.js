...
module.exports = async (ctx, project, options) => {
  const templateRemoteUri = 'https://github.com/gatewayjs/tempate/archive/master.zip';
  ...
  if (options.plugin) {
    fse.removeSync(path.resolve(projectDir, 'config'));
    fs.writeFileSync(path.resolve(projectDir, 'gateway.config.json'), '{}', 'utf8');
  }
